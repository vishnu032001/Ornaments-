...
function SuccessContent() {
  ...
  useEffect(() => {
    if (!orderId || !sessionId || getAnalyticsConsent() !== "granted") return;
    const key = `aurelia_purchase_tracked_${orderId}`;
    if (window.sessionStorage.getItem(key)) return;
    fetch(`/api/checkout/confirmation?orderId=${encodeURIComponent(orderId)}&sessionId=${encodeURIComponent(sessionId)}`)
      .then((response) => response.ok ? response.json() : null)
      .then((data) => {
        if (!data) return;
        trackPurchase(data.orderId, data.items, data.total);
  ...
  return <section className="mx-auto max-w-2xl px-6 py-24 text-center">
    ...
    <div className="mt-8 flex justify-center gap-3"><Link href="/orders" className="rounded-full border px-6 py-3">View orders</Link><Link href="/shop" className="rounded-full bg-charcoal px-7 py-3 text-white">Continue shopping</Link></div>
  </section>;
}
