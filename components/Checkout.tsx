...
export default function Checkout() {
  ...
  if (placed) {
    return <section className="mx-auto max-w-2xl px-6 py-24 text-center">
      ...
      <h1 className="mt-6 font-serif text-5xl">Order received</h1>
      <p className="mt-4 text-stone-600">Order #{placed.id.slice(-8)} has been created.</p>
      <p className="mt-2 text-sm text-stone-500">Payment is not connected yet; this order remains pending until a payment provider is integrated.</p>
  ...
  async function submitOrder(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/orders", {
        ...
        body: JSON.stringify({ email, items: items.map(({ product, quantity }) => ({ productId: product.id, quantity })) }),
      });
    ...
    <div className="mt-10 grid gap-12 lg:grid-cols-[1fr_380px]">
      <form onSubmit={submitOrder} className="space-y-8">
        ...
        <fieldset>
          ...
          <div className="mt-4 rounded-xl border p-4 text-sm text-stone-500">Payment provider integration is the next commerce phase. No card data is collected by this demo.</div>
        </fieldset>
        {error && <p role="alert" className="text-sm text-red-700">{error}</p>}
        <button disabled={loading} className="w-full rounded-full bg-charcoal py-4 text-white disabled:opacity-60">{loading ? "Creating order…" : `Place order · ${formatINR(total)}`}</button>
      </form>
